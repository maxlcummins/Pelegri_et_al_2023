...
C1 <- C1 %>% mutate(treatment = 'C1')

C1 <- B27_C1 %>% mutate(treatment = 'C1') %>% mutate(`Gene Name` = paste0( "*", `Gene Name`, "*")) %>% bind_rows(C1)

#change treatment here
mat <- C1 %>% filter(category == 'Neural') %>% select(treatment, `term description`, `Gene Name`) %>% group_by(`Gene Name`, `term description`) %>% summarise(count = n())
...
mat2 <- C1 %>% filter(category == 'Neural') %>% select(treatment, `term description`, `Gene Name`, `RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`)

mat2 <- mat2 %>% as.data.frame() %>% select(`Gene Name`, `RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`)

mat2 <- mat %>% inner_join(mat2, by = "Gene Name") %>% mutate(`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change` = as.numeric(`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`))
...
#Normalized Data
mat2$normalized = (mat2$`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`-min(mat2$`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T))/(max(mat2$`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T)-min(mat2$`RS2_3D_C1_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T))
...
pdf("Treatments_3D_DMEM_3D/new_figures/C1_chord.pdf") 
