...
B27 <- B27 %>% mutate(treatment = 'B27')

B27 <- B27_C1 %>% mutate(treatment = 'B27') %>% mutate(`Gene Name` = paste0("*", `Gene Name`, "*")) %>% bind_rows(B27)

#change treatment here
mat <- B27 %>% filter(category == 'Neural') %>% select(treatment, `term description`, `Gene Name`) %>% group_by(`Gene Name`, `term description`) %>% summarise(count = n())

#up_genes <- C1 %>% filter(category == 'Neural' & `RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change` > 0) %>% pull(`Gene Name`)

#down_genes <- C1 %>% filter(category == 'Neural' & `RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change` < 0) %>% pull(`Gene Name`)
...
mat2 <- B27 %>% filter(category == 'Neural') %>% select(treatment, `term description`, `Gene Name`, `RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`)

mat2 <- mat2 %>% as.data.frame() %>% select(`Gene Name`, `RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`)

mat2 <- mat %>% inner_join(mat2, by = "Gene Name") %>% mutate(`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change` = as.numeric(`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`))
...
#Normalized Data
mat2$normalized = (mat2$`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`-min(mat2$`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T))/(max(mat2$`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T)-min(mat2$`RS2_3D_B27_vs_RS2_3D_DMEM_log2 fold change`, na.rm = T))

mat2 %>% filter(!is.na(`Gene Name`))
...
pdf("Treatments_3D_DMEM_3D/new_figures/B27_chord.pdf") 
